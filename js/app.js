...
(function () {
  ...
  // ÚNICA MODIFICACIÓN DE ESTA TAREA: el platillo dentro de AR ahora se
  // muestra como este modelo 3D en vez de la fotografía 2D. Debe estar
  // EXACTAMENTE en esta ruta (no "3d.glb", no "dish-01.glb"). Por ahora
  // es un único modelo compartido por todos los platillos — cuando
  // existan modelos por platillo, esta ruta pasaría a leerse del propio
  // objeto `product` en vez de ser una constante fija.
  const AR_MODEL_PATH = 'assets/dishes/3D.glb';
  ...
  function renderArModel() {
    ...
    // Por ahora existe un único modelo compartido: si ya se cargó una
    // vez en esta sesión no hace falta pedirlo de nuevo al cambiar de
    // platillo (evita parpadeos innecesarios).
    if (els.arDishModel.getAttribute('src') !== AR_MODEL_PATH) {
      els.arDishModel.classList.remove('is-ready');
      els.arDishModel.setAttribute('src', AR_MODEL_PATH);
    }
  }
